...
processGroup <- function(texts, ssize = 0, max.token = 3) {
    ...
    res <- rbindlist(lapply(2:max.token, function(level) {
        data.table(token = NGramTokenizer(l.text, control = Weka_control(min = level, max = level, delimitets = " ")),
                   level = level)
    }))

    # encoding
    Encoding(res$token) <- 'unknown'
    dict <- data.table(word = NGramTokenizer(s.text, control = Weka_control(min = 1, max = 1, dilimiters = " ")))[, list(amount = .N), list(word, lword = tolower(word))]
...
train.twitter <- readRDS("train.twitter")
twitter.trained <- trainByData(train.twitter, max.size, 5)
rm(train.twitter)
saveRDS(twitter.trained, "twitter.trained")

news.trained <- trainByData(readRDS("train.news"), max.size, 5)
